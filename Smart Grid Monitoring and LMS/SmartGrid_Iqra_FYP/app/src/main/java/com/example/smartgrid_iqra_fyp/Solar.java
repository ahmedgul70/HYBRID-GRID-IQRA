...
//        TextView power = (TextView) findViewById(R.id.power);
        ...
        ampsnode = node.child("SolarAmps");
        powernode = node.child("SolarPower");
        voltnode = node.child("SolarVolt");
        loadnode = node.child("SolarState");
        ...
        ampsnode.addValueEventListener(new ValueEventListener() {
            ...
            public void onCancelled(DatabaseError error) {
                Toast.makeText(Solar.this, "Connection Error", Toast.LENGTH_LONG).show();
            }
        });
        ...
        FirebaseDatabase.getInstance().getReference(".info/connected").addValueEventListener(new ValueEventListener() {
            public void onDataChange(DataSnapshot snapshot) {
                if (((Boolean) snapshot.getValue(Boolean.class)).booleanValue()) {
                    Solar.this.mainread.setText("INTERNET CONNECTED ");
                } else {
                    Solar.this.mainread.setText("INTERNET CONNECTION FAILED");
                }
            }
